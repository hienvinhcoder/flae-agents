...
export function Dialog({ children, closeLabel = 'Close dialog', description, onClose, open, title }: DialogProps) {
  const titleId = useId();
  ...
  useEffect(() => {
    ...
    (elements.find((element) => !element.hasAttribute('data-dialog-close')) ?? elements[0])?.focus();
