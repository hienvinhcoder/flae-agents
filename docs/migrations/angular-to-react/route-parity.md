...
| `/dashboard/agents/new` | `AgentConfigComponent` in create mode | Required; component additionally permits owner/admin only | `AgentCreatePage` | No permission-loading state; only submission sets `loading`, disables the submit button, and shows its spinner | N/A | Permission failure or denied role redirects to agent list; create failure toasts and retains form | AGENT-03 |
