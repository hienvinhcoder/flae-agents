...
| Select current workspace | `PUT /users/current-workspace` | Bearer via interceptor | Automatic active workspace, already set locally | `{ workspace_id: string }` | **`any` — typed contract required** | Selection is written to the local signal/local storage before this request and retained on failure; automatic initialization updates the auth profile on success, while sidebar selection never updates that profile locally |
...
The UI currently recognizes streaming events with shapes equivalent to `{ type: 'token', text: string }`, `{ type: 'citations', citations: Citation[] }`, `{ type: 'done' }`, and `{ type: 'error', ...unknown }`. Those inferred shapes are not yet enforced by TypeScript. Both `done` and a parsed message-level `error` close and complete the service observable; the component ignores the latter in `next`, then its `complete` handler exits streaming and reloads messages without a toast or fallback. JSON parse failures and native `EventSource.onerror` instead enter the subscriber error handler.
