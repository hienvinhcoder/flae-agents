...
Member mutation has three distinct layers. Angular's `canManage` hides role, status, and removal controls for the signed-in user's own row. The backend independently forbids self role/status changes; only the owner can modify an owner/admin or transfer ownership. For removal, it rejects the owner with `400` and rejects an admin removing another admin with `403`, but currently **allows an admin to remove itself** because the self-removal branch is evaluated before the admin-target restriction. A React UI prohibition on admin self-removal preserves Angular action visibility but is frontend policy/hardening, not a backend `403`. Other missing/denied targets yield the encoded `403`/`404` outcomes.
...
| Remove member | `DELETE /workspaces/:workspaceId/members/:userUid` | Bearer via interceptor | Explicit/required, same `workspaceId` | None | `boolean` (`!!data`) | `401`; `400` owner removal; `403` workspace/role denial or admin removing another admin; `404` target missing. Backend permits admin self-removal; Angular exposes no self-remove control; local list changes only when truthy |
...
Operational security: the current SSE URL carries the Firebase token in the query string. Proxy, ingress, access, error, and application logs must redact the `token` query parameter until a React streaming transport can authenticate with a header and remove query-string credentials.
