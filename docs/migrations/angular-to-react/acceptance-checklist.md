...
- [ ] **WS-07 — completes workspace OAuth.** OAuth URL generation sends platform/optional redirect URI, callback sends code/optional state/platform, and success yields a typed workspace; propagated failures remain actionable to the caller.
- [ ] **WS-08 — handles workspace load failure.** The dashboard sync banner stops, existing/empty workspace state is retained, and workspace-dependent pages do not issue malformed requests while no workspace is active.
...
- [ ] **KB-03 — validates and uploads a document.** Upload requires a selected workspace, file, title, supported PDF/MD/TXT format, and at most 50 MB; success prepends the document, closes the modal, toasts, and starts polling; failure retains the modal and displays detail/message fallback.
- [ ] **KB-04 — creates a manual document.** Valid title/content posts the manual payload; success prepends the document, closes the modal, toasts, and starts polling; failure resets saving and preserves the input for retry.
- [ ] **KB-05 — retries failed ingestion.** Retrying a failed document disables the action, replaces that document (and open detail copy) with the returned processing document, toasts success, and resumes polling; failure preserves the failed document and displays an error.
...
- [ ] **KB-09 — types ingestion status.** The React API layer rejects an unvalidated `any` ingestion-status response and exposes the backend-agreed typed status contract.
...
- [ ] **TOPIC-07 — types topic update.** The React client replaces the current `any` update response with the backend-agreed type before the endpoint is considered migrated.
...
- [ ] **GRAPH-03 — filters graph topology.** Type filtering hides nonmatching nodes and edges whose endpoints are not both visible; physics and drawing operate on the visible topology.
