...
**Relationships:** 1 User (Owner) -> N Workspaces (hiện tại giả định 1-1 cho onboarding). 
...
1. **Về việc ánh xạ User - Workspace:**
   - A. Lưu trực tiếp `owner_uid` vào model `Workspace`. (Đơn giản nhất, phù hợp mô hình 1 owner cho đến khi có tính năng mời thành viên).
   - B. Tạo model mapping `UserWorkspace` để lưu Role. (Cân bằng, dễ scale sau này).
   - C. Lưu mảng `workspace_ids` trong collection `users`. (Dễ query list workspaces của user).
   - **Đề xuất:** Chọn B. Vì đằng nào cũng sẽ có feature "Mời thành viên", làm bảng mapping sẽ quy chuẩn hóa permission ngay từ đầu.
