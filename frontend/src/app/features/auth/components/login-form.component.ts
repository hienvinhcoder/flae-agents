...
@Component({
  ...
  template: `
    <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-soft p-8 w-full border border-white/50">
      <div class="mb-8">
        <h2 class="text-2xl font-heading font-bold text-dark-green mb-2">Đăng nhập</h2>
        <p class="text-dark-green/60 font-sans text-sm">Chào mừng trở lại! Vui lòng đăng nhập vào tài khoản của bạn.</p>
      </div>
      ...
      <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="space-y-5">
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            <label class="block text-sm font-medium text-dark-green" for="password">Mật khẩu</label>
            <a href="#" class="text-xs text-primary font-medium hover:text-dark-green transition-colors cursor-pointer">Quên mật khẩu?</a>
          </div>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <!-- Error Alert -->
        <div *ngIf="error" class="p-3 bg-red-50 text-red-600 rounded-lg text-sm flex items-start gap-2 animate-fade-in-up">
        ...
        <button 
          ...
          class="w-full py-3 px-4 bg-primary text-white rounded-xl font-medium shadow-[0_4px_14px_rgba(0,194,122,0.3)] hover:shadow-[0_6px_20px_rgba(0,194,122,0.4)] hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-[0_4px_14px_rgba(0,194,122,0.3)] flex justify-center items-center cursor-pointer"
        >
      ...
      <p class="mt-8 text-center text-sm text-dark-green/60">
        Chưa có tài khoản? 
        <a routerLink="/auth/register" class="text-primary font-medium hover:text-dark-green transition-colors cursor-pointer">Đăng ký ngay</a>
      </p>
    </div>
  `
})
