...
@Component({
  ...
  template: `
    <div class="bg-surface/80 backdrop-blur-xl rounded-2xl shadow-soft p-8 w-full border border-border/60">
      <div class="mb-8">
        <h2 class="text-2xl font-heading font-bold text-text-primary mb-2">Tạo tài khoản mới</h2>
        <p class="text-text-muted font-sans text-sm">Bắt đầu hành trình với FLAE Agent ngay hôm nay.</p>
      </div>

      <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="space-y-4">
        <!-- Full Name -->
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-1.5" for="fullName">Họ và tên</label>
          <div class="relative">
          ...
          <p *ngIf="isFieldInvalid('fullName')" class="mt-1.5 text-xs text-error animate-fade-in-up flex items-center gap-1">
            ...
            <span>Tên không hợp lệ (tối thiểu 2 ký tự)</span>
          </p>
        </div>
        ...
        <!-- Password -->
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-1.5" for="password">Mật khẩu</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
      ...
      <div class="mt-6 mb-6 relative flex items-center">
        ...
        <span class="flex-shrink-0 mx-4 text-text-muted/60 text-sm">Hoặc tiếp tục với</span>
        <div class="flex-grow border-t border-border"></div>
      </div>
      ...
      <p class="mt-8 text-center text-sm text-text-muted/80">
        Đã có tài khoản? 
        <a routerLink="/auth/login" class="text-primary font-medium hover:text-primary-hover transition-colors cursor-pointer ml-1">Đăng nhập</a>
      </p>
    </div>
  `
})
