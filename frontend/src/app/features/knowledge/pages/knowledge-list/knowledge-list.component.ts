  ...
  onUploadFile(data: { file: File; title: string; description?: string }) {
    ...
    this.apiService.uploadDocument(workspaceId, data.file, data.title, data.description).subscribe({
      ...
      error: (err) => {
        ...
        this.uploading.set(false);
        const errorMsg = err.error?.message || err.error?.detail || err.message || 'Có lỗi xảy ra khi tải tài liệu lên';
        this.toastService.error(errorMsg, 'Lỗi tải tài liệu');
      }
    });
  }
  ...
  onManualInput(data: ManualDocumentPayload) {
    ...
    this.apiService.createManualDocument(workspaceId, data).subscribe({
      ...
      error: (err) => {
        ...
        this.savingManual.set(false);
        const errorMsg = err.error?.message || err.error?.detail || err.message || 'Có lỗi xảy ra khi tạo tài liệu';
        this.toastService.error(errorMsg, 'Lỗi tạo tài liệu');
      }
    });
  }
  ...
  onDeleteDocument(id: string) {
    ...
    this.apiService.deleteDocument(workspaceId, id).subscribe({
      ...
      error: (err) => {
        ...
        this.processingAction.set(false);
        const errorMsg = err.error?.message || err.error?.detail || err.message || 'Không thể xóa tài liệu';
        this.toastService.error(errorMsg, 'Lỗi xóa tài liệu');
      }
    });
  }
  ...
  onRetryIngestion(id: string) {
    ...
    this.apiService.retryIngestion(workspaceId, id).subscribe({
      ...
      error: (err) => {
        ...
        this.processingAction.set(false);
        const errorMsg = err.error?.message || err.error?.detail || err.message || 'Không thể xử lý lại tài liệu';
        this.toastService.error(errorMsg, 'Lỗi xử lý lại');
      }
    });
  }
