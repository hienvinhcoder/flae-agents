...
import { useAgentActions, useAgentDetail } from "../hooks/use-agents";
import { createAgentCreateSchema } from "../schemas/agent-schema";
...
type SubmitErrorKey = "AGENT_CONFIG.CREATE_ERROR" | "AGENT_CONFIG.SAVE_ERROR";

interface SubmitErrorState {
  error: unknown;
  fallbackKey: SubmitErrorKey;
}
...
export function AgentConfigForm({ agentId, workspaceId }: AgentConfigFormProps) {
  ...
  const validationSchema = useMemo(
    () => createAgentCreateSchema({
      avatarColorRequired: t("AGENT_VALIDATION.AVATAR_COLOR_REQUIRED"),
      avatarIconRequired: t("AGENT_VALIDATION.AVATAR_ICON_REQUIRED"),
      modelRequired: t("AGENT_VALIDATION.MODEL_REQUIRED"),
      nameMax: t("AGENT_VALIDATION.NAME_MAX"),
      nameRequired: t("AGENT_VALIDATION.NAME_REQUIRED"),
      systemPromptRequired: t("AGENT_VALIDATION.SYSTEM_PROMPT_REQUIRED"),
      temperatureMax: t("AGENT_VALIDATION.TEMPERATURE_MAX"),
      temperatureMin: t("AGENT_VALIDATION.TEMPERATURE_MIN"),
    }),
    [t],
  );
  ...
  const submitErrorMessage = submitError ? t(submitError.fallbackKey) : null;
  ...
  return (
    <section aria-labelledby="agent-config-title" className="mx-auto grid w-full max-w-5xl gap-6">
      ...
      <form className="grid gap-6" noValidate onSubmit={(event) => void submit(event)}>
        ...
        {submitErrorMessage ? (
          <p
            className="rounded-ui-control border border-state-danger bg-state-danger-soft p-3 text-sm text-state-danger"
            role={isGloballyOwnedSubmitError(submitError?.error) ? undefined : "alert"}
          >
            {submitErrorMessage}
          </p>
        ) : null}
